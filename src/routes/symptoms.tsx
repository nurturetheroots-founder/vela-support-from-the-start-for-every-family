...
import { Phone, PhoneCall, AlertTriangle, Stethoscope, HeartPulse, MessageSquare } from "lucide-react";

export const Route = createFileRoute("/symptoms")({
  head: () => ({
    meta: [
      { title: "Symptom Checker & Red Flags — Vela" },
      { name: "description", content: "Know which postpartum symptoms need attention now, which need a call to your provider, and which are common in recovery." },
  ...
  return (
    <AppShell>
      ...
      <section className="mt-12">
        ...
        <ul className="mt-4 space-y-3">
          {sameDay.map((item) => (
            <li key={item} className="rounded-2xl border border-border/60 bg-card p-5 leading-relaxed">
              {item}
            </li>
          ))}
        </ul>
...
function HelpBanner() {
  return (
    <div className="sticky top-14 z-20 -mx-5 border-b border-primary/20 bg-primary px-5 py-3 text-primary-foreground shadow-sm">
      <p className="text-xs uppercase tracking-wider opacity-90">Talk to someone now — free, 24/7</p>
